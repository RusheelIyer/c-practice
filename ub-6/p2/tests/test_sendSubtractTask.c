...
int mq_send(mqd_t mqdes, const char *msg_ptr,
    ...
    test_assert(msg_len >= sizeof(Message) && cmd->command == CmdSubtract && cmd->parameter1 == 43 && cmd->parameter2 == 44, "mq_send: command is correct");
    (void) msg_prio;
    return 0;
}
...
int main() {
    test_start("You called mq_send in sendSubtractTask correctly.");
    test_plan(4);

    test_equals_int(sendSubtractTask(42, 43, 44), 0, "sendSubtractTask returns 0");
