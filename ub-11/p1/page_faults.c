...
VMA* _getVMA(uint32_t address)
{
    ...
    for (size_t i = 0; i < nvmas; ++i) {
        if ((_vmas[i].virtualStart <= address) && (address < _vmas[i].virtualStart + _vmas[i].length)) {
            return &_vmas[i];
        }
    }
...
int handlePageFault(uint32_t virtualBase, uint32_t pte)
{
    ...
    // ----------------
    if ((pte & PAGE_READWRITE_MASK) && !(virtualBase && PAGE_READWRITE_MASK))
        return -1;

    if (pte & PAGE_SWAPPED_MASK) {
        uint32_t physAddress = _loadFromDisk(_getVirtualBase(pte));
        pte = physAddress | (pte & PRESERVED_BITS_ON_SWAP) | PAGE_PRESENT_MASK;
...
int swapOut(uint32_t virtualBase)
{
    ...
    if (!(pte & PAGE_PRESENT_MASK)) {
        if (handlePageFault(virtualBase, pte) <= 0)
            return -1;

        pte = _getPte(virtualBase);
    }

    uint32_t physicalAddress = _getVirtualBase(pte);
    uint32_t diskOffset = _storeOnDisk(physicalAddress);

    pte = diskOffset | PAGE_SWAPPED_MASK | (pte & PRESERVED_BITS_ON_SWAP);
    _setPte(virtualBase, pte);
