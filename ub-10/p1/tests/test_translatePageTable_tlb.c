...
int main() {
	test_start("Your translatePageTable adds the address to the TLB.");
	...
	test_equals_int(translateTLB(&address, ACCESS_READ, KERNEL_MODE), 0, "translateTLB on same page succeeds");
	test_equals_int64(address, 0x2000 + 25, "address is correct");

	address = (1 * ENTRIES_PER_TABLE + 1) * (1<<OFFSET_BITS) + 28;
	test_equals_int(translateTLB(&address, ACCESS_WRITE, USER_MODE), -1, "translateTLB fails");
