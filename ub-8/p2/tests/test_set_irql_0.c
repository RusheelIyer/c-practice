...
int main() {
	test_start("Your set_irql_0() sets the right mask.");
	...
	for (int i = 0; i < 2; i++) {
		...
		test_equals_int(sigismember(&signals, SIGUSR1), 0, "SIGUSR1 is not blocked");
		test_equals_int(sigismember(&signals, SIGUSR2), 0, "SIGUSR2 is not blocked");
